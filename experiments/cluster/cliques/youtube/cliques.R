...
ggplot(datac, aes(x=factor(steps), y=runtime, fill=factor(sys))) + 
    ...
    geom_text(aes(x = factor(steps), y=max(runtime+500000), label = status, group = factor(sys)), position = position_dodge(width = 1)) +
    geom_errorbar(aes(ymin=(runtime-se), ymax=(runtime+se)),
                  colour="black", width=.005) +
    labs(title="Cliques-Youtube", x="Step", y="Runtime (ms)", fill="") +
...
ggplot(datac, aes(x=factor(steps), y=ifelse(runtime>0,runtime,1), fill=factor(sys))) + 
    geom_bar(position=position_dodge(), size=10, stat="identity") +
    geom_text(aes(x = factor(steps), y=max(runtime+50000000), label = status, group = factor(sys)), position = position_dodge(width = 1)) +
    geom_errorbar(aes(ymin=(runtime-se), ymax=(runtime+se)),
                  colour="black", width=.005) +
    labs(title="Cliques-Youtube (log)", x="Step", y="Runtime (ms)", fill="") +
    scale_colour_manual(values=cbPalette, labels=lgLabels, breaks=lgBreaks) +
    scale_y_log10() +
    theme_minimal(base_size = 20) +
    theme(legend.title=element_blank())
