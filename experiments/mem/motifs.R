...
datac <- summarySE(data, measurevar="maxmem", groupvars=c("sys", "depth"))
...
ggplot(datac, aes(x=depth, y=maxmem, colour=sys)) + 
    geom_line(size=1) +
    geom_errorbar(aes(ymin=(maxmem-se), ymax=(maxmem+se)),
                  colour="black", width=.005) +
    labs(title="Motifs w/o aggregation (Memory)", x="Enumeration Depth", y="Max Memory (GB)", fill="") +
...
ggsave(file="motifs_memory.pdf", heigh=4, width=6)
ggsave(file="motifs_memory.png", heigh=4, width=6)
