...
lgLabels <- c("GraphX", "Arabesque", "Fractal")
lgValues <- c("#d95f02", "#7570b3", "#1b9e77")
lgBreaks <- c("graphx", "arabesque", "fractalcliques")

ggplot(datac, aes(x=factor(db, levels=c("mico", "patent", "youtube", "orkut")),
                  y=runtime/1000,
                  fill=factor(sys, levels = c("graphx", "arabesque", "fractalcliques")))) + 
    ...
    labs(x="Input graphs", y="Runtime (seconds)\nlog-scale") +
