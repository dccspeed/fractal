...
ggplot(datac, aes(x=depth, y=runtime/1000, colour=sys)) + 
    geom_line(size=1) +
    geom_errorbar(aes(ymin=(runtime-se)/1000, ymax=(runtime+se)/1000),
                  colour="black", width=.005) +
    labs(title="Motifs w/o aggregation (Runtime)", x="Enumeration Depth", y="Runtime (seconds)", fill="") +
...
ggsave(file="motifs_runtime.pdf", heigh=4, width=6)
ggsave(file="motifs_runtime.png", heigh=4, width=6)
