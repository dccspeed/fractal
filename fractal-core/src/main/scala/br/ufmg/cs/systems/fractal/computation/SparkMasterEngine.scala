...
trait SparkMasterEngine [S <: Subgraph]
   ...
   def config: SparkConfiguration[S]

   var mutableConfigBc: Broadcast[SparkConfiguration[S]] = _

   def configBc: Broadcast[SparkConfiguration[S]] = {
   ...
   def parentOpt: Option[SparkMasterEngine[S]]
