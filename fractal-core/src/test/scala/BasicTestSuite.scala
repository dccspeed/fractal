...
class BasicTestSuite extends FunSuite with BeforeAndAfterAll {
   ...
   test ("[citeseer,gqueryingmcvc]", Tag("citeseer.gqueryingmcvc")) {
      // Expected output
      val numSubgraph = Map("triangles" -> 1166, "squares" -> 6059)
      ...
      val trianglesPartialResults = citeseerGraph.gqueryinmcvc(triangle.asPattern)
      ...
      // squares using MCVC
      val squaresPartialResults = citeseerGraph.gqueryinmcvc(square.asPattern)
      var totalSquares = 0L
