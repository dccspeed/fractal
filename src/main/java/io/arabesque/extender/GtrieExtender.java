...
public abstract class GtrieExtender extends Extender {
   ...
   public abstract boolean testSb(Embedding e, int pattern, int v);
   ...
   public int id(Embedding e) {
      return id(e, gtriePos);
   }
   
   public int id(Embedding e, int pos) {
      return gtrie.get(pos + 1);
   }

   public int depth(Embedding e) {
      return gtrie.get(gtriePos + 2);
   }

   public int mask(Embedding e) {
      return mask(e, gtriePos);
   }

   public int mask(Embedding e, int pos) {
      return gtrie.get(pos + 3);
   }
   
   public int parentPos(Embedding e) {
      return gtrie.get(gtriePos + 4);
   }

   public int parent(Embedding e) {
      return gtrie.get(parentPos(e));
   }

   public int nchildren(Embedding e) {
      return gtrie.get(gtriePos + 5);
   }
   
   public int childPos(Embedding e, int i) {
      if (i >= nchildren(e)) {
   ...
   public int child(Embedding e, int i) {
      return gtrie.get(childPos(e, i));
   }

   public void bootstrap(Embedding e) {
   ...
   public boolean sanityCheck(MainGraph graph, Embedding e, int pattern) {
   ...
   public int pattern(Embedding e) {
      ensurePattern(e);
     //bootstrap(e);
   ...
   private void ensurePattern(Embedding e) { 
      IntArrayList vertices = e.getVertices();
      int numVertices = vertices.size();
      int depth = depth(e);
   ...
   @Override
   public IntCollection extend(Embedding e, Computation c) {
      IntArrayList vertices = e.getVertices();
      int numVertices = vertices.size();
   ...
   private class IncludeConsumer implements IntConsumer {
      ...
      public IncludeConsumer set(IntIntMap extensions, int pattern, int id, Embedding embedding) {
      ...
      public void accept(int v) {
         if (!extensions.containsKey(v) && testSb(embedding, pattern, v)) {
   ...
   private class NeighborsPredicate implements IntIntPredicate {
      ...
      private Embedding subgraph;

      public NeighborsPredicate set(HashIntSet patterns, Embedding subgraph) {
