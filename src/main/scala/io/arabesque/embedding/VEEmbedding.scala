...
/** An edge induced embedding
  ...
  */
case class VEEmbedding(var words: Array[(Int,Int)])
  ...
  def combinations(k: Int): Iterator[VEEmbedding] = {
    words.combinations(k).map (new VEEmbedding(_))
  }
