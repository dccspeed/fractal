...
class CliquePercAggregationStorage
    extends AggregationStorage[IntArrayList,IntArrayList] {
   ...
   */
  override def aggregate(os: AggregationStorage[IntArrayList,IntArrayList])
    : Unit = {
    for ((k,_) <- os.getMapping.asScala) {
      ...
      if (!(parent equals k)) {
        UnionFindOps.union [IntArrayList] (
          v => this.getValue (v),
          (k,v) => this.aggregateWithReusables (k,v),
          parent,
          k
        )
      }
