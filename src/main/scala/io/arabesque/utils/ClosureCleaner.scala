...
object ClosureCleaner extends Logging {
  ...
  def getClassReader(cls: Class[_]): ClassReader = {
    ...
    if (resourceStream == null) {
      ...
      ClosureParser.copyStream(resourceStream, baos, true)
  ...
  def clean(
    ...
    val byteSizeBefore = SizeEstimator.estimate(closure)
    clean(closure, checkSerializable, cleanTransitively, Map.empty)
    val byteSizeAfter = SizeEstimator.estimate(closure)
    logInfo (s"Closure ${closure} byteSizeBefore=${byteSizeBefore}" +
      s"byteSizeAfter=${byteSizeAfter}")
  }
  ...
  private def ensureSerializable(func: AnyRef) {
    try {
      SparkConfiguration.serialize(func)
    } catch {
      case ex: Exception => throw new RuntimeException("Task not serializable", ex)
    }
  }
